...
var vdiCitrix = require('./CitrixWebRTC.js');
var ui = require('./ui')
...
export type WebPhoneSession = InviteClientContext &
    InviteServerContext & {
        ...
        VDIPeerConnection: typeof vdiCitrix.CitrixPeerConnection;
    };
...
export const patchSession = (session: WebPhoneSession): WebPhoneSession => {
    ...
    session.VDIPeerConnection = new vdiCitrix.CitrixPeerConnection(config);
    session.VDIPeerConnection.onaddstream = VDIhandleAddStreamEvent;
    session.VDIPeerConnection.onicecandidate = addIceEventVDI;
    session.VDIPeerConnection.onicegatheringstatechange = () => {};

    console.error('this')
    console.error(session.VDIPeerConnection);
    console.error('that')
    // Audio
    session.on('progress' as any, (incomingResponse: IncomingResponse) => {
        ...
        if (incomingResponse.statusCode === 183) {
            ...
            session.VDIPeerConnection.setRemoteDescription(
                incomingResponse.body,
                () => {
                    console.error('set remote description');
                    session.sessionDescriptionHandler.peerConnection = session.VDIPeerConnection;
                },
                () => {
                    console.error('can not set remote description');
                }
            );
    ...
    if (session.media) session.on('trackAdded', function(){
...
function VDIhandleAddStreamEvent(event) {
    ...
    ui.audio.local.srcObject = event.stream;
}

function addIceEventVDI(event) {
    console.log(event.candidate)
    if (event.candidate === null) {
        var desc = this.VDIPeerConnection.localDescription;
        console.error(desc);
    }
}
